...
FfLayerCpu::FfLayerCpu(const int in_size, const int out_size){
    ...
    for(int i=0; i<out_size; ++i){
        ...
        for(int j=0; j<in_size; ++j){
            v[j] = initializer->getRandomNumber();
        }
...
void FfLayerCpu::resetVector(ftype* v, const int size) const noexcept {
